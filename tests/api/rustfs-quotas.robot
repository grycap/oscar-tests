*** Settings ***
Documentation       Validate RustFS bucket quotas through the OSCAR API.
...
Suite Setup         Run Keywords    Check Valid OIDC Token    AND    Initialize RustFS Quota Test
Suite Teardown      Run Keywords    Cleanup RustFS Storage Enforcement Resources    AND    Cleanup RustFS Quota Buckets    AND    Restore Original RustFS Quota


*** Variables ***
${RUSTFS_STORAGE_PER_BUCKET}   50Mi
${BUCKET_A}                    ${EMPTY}
${BUCKET_B}                    ${EMPTY}
${CLI_CLUSTER}                 ${EMPTY}
${ORIGINAL_RUSTFS_BUCKETS_MAX}   ${EMPTY}
${ORIGINAL_RUSTFS_STORAGE_MAX}   ${EMPTY}
${QUOTA_SERVICE_NAME}          ${EMPTY}
${QUOTA_SERVICE_BUCKET}        ${EMPTY}
${SMALL_OBJECT_FILE}           ${DATA_DIR}/rustfs-quota-small.bin
${LARGE_OBJECT_FILE}           ${DATA_DIR}/rustfs-quota-large.bin
${RETRY_OBJECT_FILE}           ${DATA_DIR}/rustfs-quota-retry.bin


*** Test Cases ***
OSCAR RustFS Quotas Are Reported
    [Documentation]    /system/quotas/user exposes RustFS bucket count and storage quota information.
    ${payload}=    Fetch User Quotas Payload
    Dictionary Should Contain Key    ${payload}    minio
    ${rustfs}=    Get From Dictionary    ${payload}    minio
    Dictionary Should Contain Key    ${rustfs}    buckets
    Dictionary Should Contain Key    ${rustfs}    storage_per_bucket
    Dictionary Should Contain Key    ${rustfs}    storage_total
    Dictionary Should Contain Key    ${rustfs["buckets"]}    max
    Dictionary Should Contain Key    ${rustfs["buckets"]}    used
    Dictionary Should Contain Key    ${rustfs["storage_per_bucket"]}    max
    Dictionary Should Contain Key    ${rustfs["storage_total"]}    used

OSCAR Enforces RustFS Bucket Count Quota
    [Documentation]    Creating one more bucket than the configured RustFS bucket quota is rejected.
    ${payload}=    Fetch User Quotas Payload
    ${used}=    Evaluate    int($payload["minio"]["buckets"]["used"])
    ${limit}=    Evaluate    ${used} + 1
    Update RustFS Quota    ${limit}    ${RUSTFS_STORAGE_PER_BUCKET}
    ${response}=    Create RustFS Bucket    ${BUCKET_A}
    Should Be Equal As Strings    ${response.status_code}    201
    ${response}=    Create RustFS Bucket    ${BUCKET_B}
...
OSCAR Applies RustFS Storage Quota To New Buckets
    ...
    Should Be Equal As Strings    ${bucket["storage_quota"]["max"]}    ${RUSTFS_STORAGE_PER_BUCKET}

OSCAR Enforces RustFS Storage Per Bucket Quota
    [Documentation]    RustFS rejects further object uploads after its scanner observes that a bucket exceeded the configured quota.
    [Tags]    storage-enforcement    oscar-cli
    Ensure OSCAR CLI Cluster Configured
    ${payload}=    Fetch User Quotas Payload
    ${limit}=    Evaluate    int($payload["minio"]["buckets"]["used"]) + 1
    Update RustFS Quota    ${limit}    1Mi
    Create Quota Test Files
    Create RustFS Quota Service
    ...
    Should Be Equal As Integers    ${large_result.rc}    1


*** Keywords ***
Initialize RustFS Quota Test
    [Documentation]    Prepare unique bucket names and capture the current RustFS quota for restoration.
    ${suffix}=    Generate Random String    8    [LOWER][NUMBERS]
    Set Suite Variable    ${BUCKET_A}    robot-rustfs-quota-${suffix}-a
    Set Suite Variable    ${BUCKET_B}    robot-rustfs-quota-${suffix}-b
    Set Suite Variable    ${CLI_CLUSTER}    robot-rustfs-quota-${suffix}
    Set Suite Variable    ${QUOTA_SERVICE_NAME}    robot-rustfs-quota-svc-${suffix}
    Set Suite Variable    ${QUOTA_SERVICE_BUCKET}    robot-rustfs-quota-svc-${suffix}
    Cleanup RustFS Quota Buckets
    ${payload}=    Fetch User Quotas Payload
    ${has_rustfs}=    Evaluate    "minio" in $payload
    Skip If    not ${has_rustfs}    This OSCAR deployment does not expose RustFS quota information.
    Set Suite Variable    ${ORIGINAL_RUSTFS_BUCKETS_MAX}    ${payload["minio"]["buckets"]["max"]}
    Set Suite Variable    ${ORIGINAL_RUSTFS_STORAGE_MAX}    ${payload["minio"]["storage_per_bucket"]["max"]}
...
Update RustFS Quota
    [Documentation]    Update RustFS bucket-count and per-bucket storage quotas for the current user.
...
Create RustFS Bucket
    [Documentation]    Create a private RustFS bucket through the OSCAR buckets API.
...
Create RustFS Quota Service
    [Documentation]    Create a temporary service with one RustFS bucket used for input and output.
...
Service Bucket Should Have Storage Quota
    [Documentation]    Assert that the service bucket has the expected RustFS storage quota before uploading objects.
...
RustFS Should Reject Additional Upload
    [Documentation]    Try another upload after quota overuse is visible; RustFS scanner enforcement is best-effort and delayed.
    ...
    Log    ${result.stdout}
    Log    ${result.stderr}
    Should Not Be Equal As Integers    ${result.rc}    0    msg=Quota is configured and exceeded, but RustFS still accepted an additional upload within the timeout.
...
Cleanup RustFS Storage Enforcement Resources
...
Restore Original RustFS Quota
    [Documentation]    Restore the RustFS quota values captured at suite startup.
    Run Keyword If    '${ORIGINAL_RUSTFS_BUCKETS_MAX}' != '' and '${ORIGINAL_RUSTFS_STORAGE_MAX}' != ''
    ...    Update RustFS Quota    ${ORIGINAL_RUSTFS_BUCKETS_MAX}    ${ORIGINAL_RUSTFS_STORAGE_MAX}
