...
Verify Service isolation_level SERVICE Creation
    ...
    Should Not Match Regexp    ${output}    "bucket_name":"${bucket_name}-${id} ","visibility":"private"
    ${response}=    Verify Second Bucket
    ${output} = 	Convert To String 	${response.status_code}
    Should Not Match Regexp    ${output}    "bucket_name":"${bucket_name}-${id2}","visibility":"private"
...
Verify isolation_level SERVICE -> USER Update
    ...
    Should Be Equal As Strings    ${response.status_code}    200
    Should Contain    ${response.content}    "bucket_name":"${bucket_name}","visibility":"private"
    Should Contain    ${response.content}    "bucket_name":"${bucket_name}-${id}","visibility":"private"
    ${response}=    Verify Second Bucket
    ${output} = 	Convert To String 	${response.status_code}
    Should Not Match Regexp    ${output}    "bucket_name":"${bucket_name}-${id2}","visibility":"private"
...
Verify isolation_level USER -> USER Update with more users
    ...
    Should Contain    ${response.content}    "bucket_name":"${bucket_name}-${id}","visibility":"private"
    ${response}=    Verify Second Bucket
     Should Contain    ${response.content}    "bucket_name":"${bucket_name}-${id2}","visibility":"private"
