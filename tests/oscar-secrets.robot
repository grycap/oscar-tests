...
OSCAR Create Service with Secret
...
OSCAR Service with Secret Invoke Asynchronous 
...
OSCAR Service with Secret Update Service
...
OSCAR Service with Secret Invoke Asynchronous Service
    [Documentation]    Invoke the asynchronous service
    ${body}=    Get File    ${INVOKE_FILE}
    ${response}=    POST    url=${OSCAR_ENDPOINT}/job/${SERVICE_NAME}    expected_status=201    data=${body}
    ...    headers=${HEADERS}
...
OSCAR Service with Secret Update Service Again
    [Documentation]    Update a service
...
OSCAR Service with Secret Invoke Asynchronous Service Again
    [Documentation]    Invoke the asynchronous service
    ${body}=    Get File    ${INVOKE_FILE}
    ${response}=    POST    url=${OSCAR_ENDPOINT}/job/${SERVICE_NAME}    expected_status=201    data=${body}
    ...    headers=${HEADERS}
...
OSCAR Service with Secret Delete Service
